...
The UN member states data frame is included in the `moderndive` package. To keep things simple, we `select()` only the subset of the variables needed here, and save this data in a new data frame called `UN_data_m10`. Note that the variables used are different than the ones chosen in Chapter \@ref(regression). We also set the `income` variable to be a `factor` so that its levels show up in the expected order.
